    ...
    <View style={styles.container}>
      <View style={styles.tabContainer}>
        {tabs.map(tab => (
          <TouchableOpacity
            key={tab}
            style={[styles.tabButton, activeTab === tab && { backgroundColor: Colors.Blanco, borderWidth: 1, borderColor: Colors.GrisClaroPeroNoTanClaro, borderBottomWidth: 0 }]}
            onPress={() => setActiveTab(tab)}
          >
            <Text style={[styles.tabText, activeTab === tab && { fontWeight: "bold", backgroundColor: Colors.GrisClaro }]}>{tab}</Text>
          </TouchableOpacity>
        ))}
      </View>
